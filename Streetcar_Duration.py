...
# Load your data
data = pd.read_csv('streetcar-data.csv')
...
xgb_model = XGBRegressor(
    ...
    learning_rate=0.2,
    random_state=42,
    reg_lambda=20,      # L2 regularization (shrinks large weights)
    reg_alpha=5,        # L1 regularization (sparsity)
...
plt.title('Streetcar Feature Importances from XGBoost Model')

# Rotate x-axis labels diagonally
plt.xticks(rotation=45, ha='right')  # Rotate by 45 degrees and align text to the right

plt.tight_layout()
plt.savefig('Street_XGBoost_Importance.png')
plt.show()

# Check correlation between 'Min Gap' and 'Min Delay'
